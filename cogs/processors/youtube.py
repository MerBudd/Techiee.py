...
class YouTubeProcessor(commands.Cog):
    ...
    async def process(self, message, url, cleaned_text, settings, reply_chain_context=None):
        ...
        response_text, user_parts = await process_youtube_url(url, cleaned_text, settings, history, user_display_name, user_username, history_key=history_key)
        
        # Define retry callback
        async def retry_callback():
            result, _ = await process_youtube_url(url, cleaned_text, settings, history, user_display_name, user_username, history_key=history_key)
            return result
        
        # Define history update callback
        async def update_history(response_text):
            if dynamic_config.max_history > 0 and user_parts is not None:
