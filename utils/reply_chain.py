...
async def format_message_for_context(message: discord.Message) -> Content:
    ...
    if message.author.bot:
        ...
        parts.append(Part(text=f"[CONTEXT — Bot's previous response at {timestamp}]\n{text}"))
        ...
        await _add_embed_parts(message, parts)
        
        return Content(role="model", parts=parts)
    else:
        # User messages
        text = message.content if message.content else "[No text content]"
        formatted_text = f"[CONTEXT MESSAGE from {display_name} (@{username}) at {timestamp}]:\n{text}"
        ...
        # Stickers (downloads actual image)
        await _add_sticker_parts(message, parts)
        
        # GIFs and embeds (downloads GIF thumbnails)
        await _add_embed_parts(message, parts)
    ...
    if message.stickers:
        async with aiohttp.ClientSession() as session:
            for sticker in message.stickers:
                ...
                # Try to download the actual sticker image
...
def format_reply_chain_as_context_text(chain_contents: list) -> str:
    ...
    context_parts = []
    context_parts.append("--- Reply Chain Context (messages from other users that were replied to) ---")
    
    for content in chain_contents:
        if content.parts:
            for part in content.parts:
                if hasattr(part, 'text') and part.text:
                    role_indicator = "[Bot's Response]" if content.role == "model" else "[User Message]"
